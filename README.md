...
Dialog appearance is driven by [`@teqbench/tbx-mat-severity-theme` ↗](https://github.com/teqbench/tbx-mat-severity-theme). Importing `tbx-mat-dialogs` styles emits `--tbx-mat-dialog-{level}-{background,text}` aliases for all six severity tiers, plus per-package opacity tokens used by the severity-aware action button variants. The dialog overlay's `panelClass` includes a per-severity class (`.tbx-mat-dialog-panel-{level}`) that drives the entire surface via the package's `_severity-panel` SCSS mixin: it sets [Angular Material ↗](https://material.angular.dev)'s `mat.dialog-overrides({ container-color, subhead-color, supporting-text-color })` so the dialog container, title, and body all take the severity colors, and pipes the same tokens into the `mat.button-overrides()`, `mat.form-field-overrides()`, and `mat.divider`/`mat.checkbox`/`mat.slide-toggle`/`mat.radio`/`mat.button-toggle`/`mat.chips` overrides so every Material control inside the panel reads correctly against the colored surface.
...
The dialog overlay panel uses `MatDialog`'s surface element. The package's global SCSS scopes a small set of structural rules plus per-severity color rules:

- `.tbx-mat-dialog-panel .mat-mdc-dialog-container .mat-mdc-dialog-surface` — removes [Angular Material ↗](https://material.angular.dev)'s default padding (the shell manages its own via `--dialog-padding-inline`) and rounds the corners to `1rem`.
- `.tbx-mat-dialog-panel-{default|success|error|warning|information|help}` — applies the `_severity-panel` mixin, which colors the entire surface (container background, title, body text, dividers, footer buttons, form controls) with the active severity's tokens.
